...
async function cargarDatos() {
  try {
    [alertasCache, alumnosCache] = await Promise.all([
      apiCall('/backend/api/alertas.php'),
...
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    await apiCall('/backend/api/alertas.php', 'POST', payload);
...
async function marcarAtendida(id) {
  try {
    await apiCall('/backend/api/alertas.php', 'PUT', { id, atendida: true });
...
async function eliminarAlerta(id) {
  ...
  try {
    await apiCall('/backend/api/alertas.php?id=' + id, 'DELETE');
