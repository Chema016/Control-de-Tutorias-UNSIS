...
async function cargarDatos() {
  try {
    [alumnosCache, profesoresCache] = await Promise.all([
      apiCall('/backend/api/alumnos.php'),
      apiCall('/backend/api/profesores.php'),
    ]);
    llenarSelectsTutor();
    renderTabla();
  } catch (err) {
...
function abrirEditarAlumno(id) {
  ...
  apiCall('/backend/api/alumnos.php', 'PUT', { id: a.id, nombre: nuevoNombre.trim() })
...
async function eliminarAlumno(id) {
  ...
  try {
    await apiCall('/backend/api/alumnos.php?id=' + id, 'DELETE');
...
async function importarAlumnos() {
  ...
  for (const row of importRows) {
    ...
    try {
      await apiCall('/backend/api/alumnos.php', 'POST', payload);
      ok++;
    } catch(e) {
      fail++;
    }
