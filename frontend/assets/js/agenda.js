...
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    if (id) {
      payload.id = id;
      await apiCall('/backend/api/citas.php', 'PUT', payload);
      showMsg('msg', 'Cita actualizada correctamente', 'success');
    } else {
      await apiCall('/backend/api/citas.php', 'POST', payload);
...
async function eliminarCita(id) {
  ...
  try {
    await apiCall('/backend/api/citas.php?id=' + id, 'DELETE');
