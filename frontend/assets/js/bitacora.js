...
async function cargarDatos() {
  try {
    [bitacoraCache, alumnosCache] = await Promise.all([
      apiCall('/backend/api/bitacora.php'),
...
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    if (id) {
      payload.id = id;
      await apiCall('/backend/api/bitacora.php', 'PUT', payload);
      showMsg('msg', 'Registro actualizado correctamente', 'success');
    } else {
      await apiCall('/backend/api/bitacora.php', 'POST', payload);
...
async function eliminarBitacora(id) {
  ...
  try {
    await apiCall('/backend/api/bitacora.php?id=' + id, 'DELETE');
