...
async function cargarDatos() {
  try {
    [canalizacionesCache, alumnosCache] = await Promise.all([
      apiCall('/backend/api/canalizacion.php'),
...
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    if (id) {
      payload.id = id;
      await apiCall('/backend/api/canalizacion.php', 'PUT', payload);
      showMsg('msg', 'Canalización actualizada correctamente', 'success');
    } else {
      await apiCall('/backend/api/canalizacion.php', 'POST', payload);
...
async function eliminarCanalizacion(id) {
  ...
  try {
    await apiCall('/backend/api/canalizacion.php?id=' + id, 'DELETE');
