...
async function cargarProfesores() {
  try {
    profesoresCache = await apiCall('/backend/api/profesores.php');
...
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    if (id) {
      payload.id = id;
      await apiCall('/backend/api/profesores.php', 'PUT', payload);
      showMsg('msg', 'Profesor actualizado correctamente', 'success');
    } else {
      if (!password || password.length < 6) {
        throw new Error('La contraseña debe tener al menos 6 caracteres');
      }
      await apiCall('/backend/api/profesores.php', 'POST', payload);
...
async function eliminarProfesor(id) {
  ...
  try {
    await apiCall('/backend/api/profesores.php?id=' + id, 'DELETE');
